...
public class SimpleUDPClient {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            socket.setSoTimeout(0); // blokirajući recv
            InetAddress host = InetAddress.getByName("localhost");
            int port = 7000;

            int[] sizesKiB = {1, 2, 4, 8, 16, 32}; //128kb is too much, just loading
            ...
            for (int size : sizesKiB) {
                ...
                for (int i = 0; i < 1000; i++) {
                    ...
                    // opcionalno: parsiraj odgovor (proveri number)
                    Message reply = Message.deserialize(replyPayload);
                    // sanity check (nije obavezno): reply.getNumber() == i+1
